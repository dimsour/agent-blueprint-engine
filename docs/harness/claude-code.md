...
Adapter `claude-code` (planned, roadmap P2). Source entities are defined in `packages/core/src/schema`.
...
| Skills (`skills[]`)                                                                                                   | `.claude/skills/<id>/SKILL.md` + `resources[]` as sibling files; `activation.filePatterns` + `directories` → `paths:`; `activation.intents` + `whenToUse` → `when_to_use`; `allowedToolIds` → `allowed-tools`                                                                                                                                             | native                                                                 |
| Non-primary agents                                                                                                    | `.claude/agents/<id>.md`: `description`, `tools` (from `toolIds`), `model` (from `model.preference`: fast → `haiku`, balanced → `sonnet`, strong → `opus`; `hint` wins when set), `skills` (from `skillIds`), `memory: project` when `memoryIds` is non-empty, `permissionMode` derived from permissions (all allow → `acceptEdits`, any ask → `default`) | native                                                                 |
...
| Hooks (`hooks[]`)                                                                                                     | `.claude/settings.json` → `hooks` (table below)                                                                                                                                                                                                                                                                                                           | native                                                                 |
| Gates (`gates[]`) with executable `criteria[].command`                                                                | `Stop` and `SubagentStop` hooks running the command; `onFail: block` → exit 2 semantics; `request-approval` → `decision: block` + reason asking for approval                                                                                                                                                                                              | native                                                                 |
...
Action lowering: `command` / `run-tests` / `format` / `lint` / `secret-scan` → `{ "type": "command", "command": <action.command>, "timeout": <timeoutSec> }`; `prompt-check` → `{ "type": "prompt", "prompt": <action.prompt> }`; `check-iron-laws` → `{ "type": "prompt", "prompt": <generated from the iron laws in scope> }`. `conditions.filePatterns` are appended to the command as a guard comment and enforced by the command itself (the harness has no file glob matcher on hooks; verify). `onFailure: block` → the command must exit 2 on failure; `warn` → exit 0 with `systemMessage`; `return-to-agent` → exit 2 with the output on stderr.
...
- Hook file-pattern conditions are not native; they are enforced inside the generated command.
