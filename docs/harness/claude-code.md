...
Action lowering: `command` / `run-tests` / `format` / `lint` / `secret-scan` → `{ "type": "command", "command": <wrapped action.command>, "timeout": <timeoutSec> }`; `prompt-check` → `{ "type": "prompt", "prompt": <action.prompt> }`; `check-iron-laws` → `{ "type": "prompt", "prompt": <generated from the iron laws in scope> }`. A prompt hook is not read by the agent: Claude Code sends "the hook input and your prompt to a Claude model, Haiku by default", which answers `{"ok": true}` or `{"ok": false, "reason"}`, and on `Stop` the reason "is fed back to Claude as its next instruction". So the generated prompt is written for that judge — allow unless the transcript shows a law broken; unfinished work and checks nobody ran are not violations — where the reminder printed for Copilot is written for the agent. Worded as a reminder, the judge refused to certify what it could not see and blocked every stop of an unfinished task.
...
On `Stop` and `SubagentStop` a blocking command blocks once per turn. The hook input there carries `stop_hook_active`, "`true` when Claude Code is already continuing as a result of a stop hook. Check this value or process the transcript to avoid blocking on a condition that will never resolve. Claude Code overrides the hook and ends the turn after 8 consecutive blocks." So the wrapper on those events reads its input first, passes it on to the command, and when the field is true exits 1 instead of 2 on failure, with "Still failing after the agent was sent back once; not blocking again this turn." before the output. The command still runs — a fix is worth verifying — but a scanner that is not installed, or a test that the agent cannot make pass, is reported to the user rather than sending the agent round eight times. The Iron Law judge prompt says the same: `stop_hook_active` true → ok.
