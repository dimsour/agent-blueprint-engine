...
- **Secret scan before a push** (built, P7-04; `apps/web/src/lib/secret-scan.ts`): every generated and source file about to be committed is scanned for common credential patterns (AWS access keys, GitHub `ghp_`/`github_pat_`, OpenAI `sk-`, Anthropic `sk-ant-`, private key blocks, JWT-looking strings, generic `api[_-]?key\s*[:=]\s*\S{16,}`). A hit blocks the action with the file and line, and the user can override per finding. The finding shows the line with the match masked: a screenshot of the warning must not become the leak the warning exists to prevent. The export runs the same scan on the same files and refuses on the same terms (P7-06): an archive is the other way a project leaves the browser, and it usually leaves it towards somebody else. Both surfaces share one component, so the list, the per-finding override and the rule that nothing proceeds until every finding is ticked cannot drift apart. A file that is not text is skipped rather than searched (P8-04): it has no lines to report, and looking anyway would only pretend to have looked. A credential inside an image is out of scope for this scan and stated as such rather than implied to be covered.
...
- [x] Export and push paths call the secret scanner and surface hits before writing (P7-04, P7-06).
