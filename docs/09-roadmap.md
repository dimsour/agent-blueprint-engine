...
- This is not "new from template", which fills a whole artifact from the 36 artifact templates and stays as it is. This fills one field, for the person who knows what they want everywhere except here.
...
- Depends on: nothing
- Description: Reported from use while checking P9-10: the findings were missing everywhere except the evaluation view, and came back as soon as anything was edited. Not a regression from P9-10 — a defect from P3, dated `157aa1e`. `load` did `diagnostics: diagnostics ?? validateBlueprint(blueprint)`, and the caller passes the **reader's** diagnostics, which for a project that reads cleanly is `[]`. An empty array is not nullish, so the fallback never ran and the whole validation pass was skipped. The health bar, the inspector and the export blockers all read the store, so all three showed nothing; the evaluation view runs its own pass, which is why it alone looked right. The first keystroke started the debounced revalidation and everything appeared at once.
...
- Reported from the first real run, fixed: a Blueprint whose thirty laws all applied to everything and were listed by no agent showed thirty orphan warnings, and the model duly set every scope to "all" — which was already true — with one reason copied twelve times, and raised a severity on the way. Three fixes. **Core:** `findOrphans` now takes the Blueprint and counts a law's or rule's own scope as a use, since that is what the compiler compiles (`lawsForAgent`); a law is an orphan only when its scope reaches nothing and no agent lists it, which is also `BP-LAW-001`'s condition. **Operation:** the connections section says what each law's scope reaches, and named artifacts go through `draftsFrom`, so the catalogue's field list holds and the severity change is refused with a note. **Prompt:** what a scope is for, read what each law governs and who does that work, apply to all only what is universal, and a blanket decision is no decision.
- Reported from use, fixed: "where do I see this scope option?" — nowhere. `scope` and `enforcement` were in the model, the docs and the tutorial, and on no form; the YAML tab was the only way. The Iron Law form now has Scope (every agent, or named agents and workflows) and Enforcement; the rule form has Scope. The inspector's Used by says "applies to every agent" for a global law instead of "nothing refers to this".
...
- Open, found on the way: the compiler reads a law's and a rule's `scope` and never an agent's `ironLawIds` or `ruleIds`, while the model, the docs and the editor present both as bindings. A law listed by an agent but scoped to nothing is not compiled for that agent. Either the compiler unions the two or the agent lists go — a decision for an ADR, not for this task.
