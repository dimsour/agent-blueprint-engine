...
| `BP-ORPHAN-001`, `BP-ORPHAN-002`, `BP-ORPHAN-003`, `BP-ORPHAN-004`, `BP-ORPHAN-005`, `BP-ORPHAN-006`, `BP-ORPHAN-007`, `BP-ORPHAN-008` | warning                             | Unused skill (001), workflow (002), iron law (003), rule (004), gate (005), tool (006), reference (007), memory (008), computed by `findOrphans(graph, blueprint)` (see §5). A law or rule counts as used by its own scope, so 003 and 004 fire only when the scope reaches nothing and no agent lists it — the same condition `BP-LAW-001` describes from the law's side. |
...
| `BP-CODEX-003`                                                                    | error    | Plugin layout only: a skill or workflow has the id `guide`, which the plugin's instruction skill takes (P9-28).        |
| `BP-COPILOT-003`                                                                  | error    | Plugin layout only: a path-scoped rule has the id `guide`, which the plugin's instruction file takes (P9-36).          |
...
`findOrphans(graph, blueprint)` returns entities of `ORPHANABLE_KINDS` nothing uses. For a skill, a gate, a tool, a reference or a memory that means no dependents. An Iron Law or a rule is different, because the compiler reads its own `scope` — every agent's instructions when `scope.all`, the named agents' and workflows' otherwise — and never the agents' `ironLawIds`/`ruleIds` (`lawsForAgent`, `rulesForAgent`). So a law is an orphan only when its scope reaches nothing **and** no agent lists it; a law that applies to all is in every compiled file and is not one, whatever the agents list. Until P9-40 the check counted dependents alone, and the fixture of that task — thirty laws all `scope.all`, none listed — was thirty warnings whose remedy ("set it to apply to everything") was already true.
