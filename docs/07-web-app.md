...
When a pushed target was packaged as a plugin (P9-27), the success panel shows the install commands with the repository just pushed to filled in — `/plugin marketplace add <owner>/<repo>` and `/plugin install <id>@<id>` — since the dialog is the one place that knows the name.
