...
| `/settings`                                             | P3 (storage), P6 (AI), P7 (GitHub) | What this browser is holding: stored projects, space used, and a way to remove them. The editor's preferences follow: whether an edit saves itself (P9-37). The AI endpoint sits here — provider preset, base URL, model, key, where the key is kept, and a Save-and-test that really calls the endpoint and reports what came back. The GitHub token sits beside it on the same terms, with a Save-and-check that asks GitHub who the token belongs to, and Sign in with GitHub when the deployment has an OAuth app. |
| `/tutorial`                                             | P9-06, P9-38                       | How it works: the docs/00 story, step by step, with a screenshot of each, then every kind of artifact on its own — definition, fields, source file, what each harness gets. Reads nothing, so it works before there is a project                                                                                                                                                                                                                                                                                       |
...
### The tutorial (`/tutorial`, P9-06, extended in P9-38)
...
The loop uses every kind of artifact once and says nothing about what is inside one, so a second half, **The parts** (P9-38), takes them one at a time: the project on disk, agents, permissions, skills, workflows, iron laws and rules, hooks, gates, tools with references and memory, requirements with scenarios, the assistant, the compile targets, and GitHub (the last two artifacts of the loop, added in P9-39, so that the AI layer and the push are explained and not only pressed). Each part is the definition docs/00 fixes, the control that adds one, the settings on its form and what they decide (from docs/02, in the form's own labels), the source file the project writes for it — a starter's file or a marked excerpt of one; the one exception, an MCP tool no starter ships, says so in its caption — and a table of what each harness gets, in the words of the adapters' capability matrices (docs/04). The technical claims are the ones the code makes: the twelve permission operations and the Claude rules they lower to, the hook exit-code convention and the once-per-turn stop guard, the sixteen step types, the seven requirement checks. The page has two `h2`s, the loop and the parts, with the steps and the parts as `h3`s beneath them.

The contents travel with the reader (P9-39): a list at the top of a page this long is out of reach after the first screen. `TutorialNav`, the one client component on the route, is a sidebar that stays beside the text from `lg` up — both halves, numbered, with the section on screen marked `aria-current` — and below `lg` a bar that stays at the top with the same sections in a `<select>` labelled "Jump to", which also shows the section on screen. Which section that is comes from one `IntersectionObserver` over a band across the upper part of the viewport: the first section in reading order that crosses it is current, so a long section stays current while its pictures scroll past and the next takes over when its heading arrives. Every section carries a `scroll-mt` for the bar so a jump does not land under it. A jump replaces the address rather than pushing it: the links are real, so a middle click or a copied address still gets the section, but a plain click leaves no trail of `#hook`, `#gate`, `#github` for **Back** to walk through before it reaches the page the reader came from — Back on the tutorial is the dashboard, or wherever it was opened from, as on every other route (P9-02).
...
Its twenty-one pictures come from `pnpm --filter web screenshots`, which drives the real app and writes into `apps/web/src/assets/screenshots`; the page imports them through `next/image` and the README links the same files. **A shot the tutorial needs and the script does not take is added to the script, never captured by hand** — that is the whole reason they are generated. Two steps of the story need credentials the reader may not have, and both are shown honestly rather than faked: "Review" is a template proposing a change, which is the same bargain as an AI proposal without needing an endpoint, and "Improve" is the assistant saying it has no endpoint yet. The five pictures of the parts are the forms of the same starter — the agent, its permission grid, a law, a hook and a gate — because the detail the parts explain is what the forms show. The assistant and GitHub are pictured as their Settings cards, which is as far as a screenshot without a key or a token can honestly go.

The axe sweep covers the route in both themes, which is the check that matters on a page that is mostly pictures: an image without a description is a violation. The parts added a second kind of content it catches: a code block that scrolls sideways is a scrollable region, and one a keyboard cannot reach is a violation, so each is focusable and named by the path of the file it shows.
...
| Project | IndexedDB                         | Autosaved through `renderProjectFiles` about a second after the last edit, and on Save. Leaving a project writes its pending edit first. Autosave can be switched off in Settings (P9-37); then only Save writes |
...
Autosave is a preference (`lib/editor-settings`, `ab:settings:editor`, on by default), read by the store at the moment an edit lands, so a change in Settings applies to the next keystroke. Off, an edit is written by Save (⌘S) and by nothing else: the top bar says "Unsaved" until then, closing or reloading the tab asks first (`beforeunload`, only when there is something unsaved and autosave is off), and leaving the project inside the app drops the edit — which is what switching autosave off asks for, and what the setting's help text says (P9-37).
...
- **How to fix a finding** (P9-10): every finding — in the health bar, in the inspector, in an evaluation dimension, in the list blocking an export — carries a control named `How to fix <code>`. Hovering or focusing it gives the code's one-line meaning; pressing it unfolds the remedy underneath and leaves it there. Both come from `DIAGNOSTIC_CODES` in core, so the guidance is written once per code and cannot differ between the four surfaces. A code the catalogue does not know renders no control rather than an empty panel. It draws a question mark, not an ⓘ: an info-severity finding already draws an ⓘ at the head of its own row (P9-12).
- **Where to look** (P9-19): a finding is filed against one artifact and its cause is often another — a requirement fails because of a hook, a contradiction has a second half, a broken reference has a target. That has always been `related` on the diagnostic, and nothing rendered it. Every findings list now shows it as a row of chips under the message, `Hook: secret-scan-before-stop`, each one navigating to that artifact; when the finding says why it is related (a near miss carries its reason, docs/05 §4) the reason sits beside the first chip and is the tooltip on every one. The evaluation view's own check list shows the same thing per failed check, as `nearest:`.
- **Every option explains itself** (P9-21): each fixed choice a form offers — an agent's role, a hook's trigger and action, a gate's criterion kinds, a workflow step's type, a reference's kind, a memory scope, a check type — shows what choosing it does. In the open list every item carries its sentence under its value; under the closed control the chosen one's sentence stays visible, so what is picked is always explained without opening the list. The value itself stays the raw enum word, in `font-mono`, so the form shows what the file will say. The text comes from `packages/core/src/model/options.ts`, which is also what docs/02's options reference is generated from and what the permissions grid uses for its row labels — one table, three readers, and a test that every enum value has an entry.
- **Findings on the form** (P9-20): the finding is also shown under the control that fixes it. `lib/field-findings.ts` maps the store's diagnostics onto the selected artifact's fields — a finding filed against the artifact marks the fields its code names (`DiagnosticCode.fields`, docs/05 §2), and a finding elsewhere whose near miss names this artifact marks the field the near miss recorded, with the clause as its text and a link back: `BP-REQ-001 its action is command, not secret-scan — from requirement: security-enforcement` under the hook's Action select. Every `Field` accepts `hints`; the form spreads `hint('description')` beside `exampleFor(kind, 'description')`, the same shape as the examples. A dotted path — `action.command` — names a control inside an object. Not a live region: fields re-validate on every keystroke, and a screen reader announcing the same finding on each is worse than one it can tab to.
- **Fix with AI** (P9-12): beside it, `Fix <code> with AI` opens a dialog that asks a model to clear that one finding, through `fixFinding` (docs/06). The dialog shows the instructions being sent — the same `remedy`, verbatim — before it asks, so the answer can be judged against the request, and what comes back goes to the ordinary per-op review; nothing is applied until an op is accepted. The control is absent, rather than present and useless, for the codes `fixabilityOf` refuses: files on disk, ids that need the rename refactor, targets that need enabling. Both controls are 24px targets with a 14px icon centred in them.
- **Evaluation view**: per-dimension score with its findings and weight, requirement results with each check and the artifacts that satisfied it, and a button to recompute. Every finding navigates. Each dimension with more than one fixable finding carries **Fix all N**, which sends them to `fixFindings` in a single ask (P9-17) — not a loop over the row-level control, because two findings so often name the same artifact that fixing them separately means two edits of one file and one of them lost. Below two, the rows' own controls already do it and a second button would say the same thing twice. **Run AI analysis** adds a model's contradictions and its verdicts on `ai-judged` requirement checks beside the rules', badged `AI`, deduplicated against what the validator already said — and explicitly not counted in the score, because a number that moves when nothing about the Blueprint did is a number nobody can act on.
- **Compatibility view**: concept by harness matrix from the adapters' own capability matrices, filtered to the concepts the Blueprint uses, each cell carrying the adapter's explanation. Targets are toggled from here, and a harness with a plugin layout has a **Package as a plugin** switch under them (P9-27), which writes `layout: plugin` into the target's options — off removes the option rather than writing `project` — and the matrix and notes change with it, since a plugin cannot carry what a project can.
...
**Pushing** (`⌘K` → Push to GitHub, or the GitHub button in the top bar) is two steps, and the second is a preview. Choose a repository (`owner/name`, a URL, or one of the hundred the token most recently touched) and a branch, which is created by the push when it does not exist; a name the token cannot list is offered for creation as soon as it is typed — empty, private by default, so the Blueprint is its first commit — with Preview still beside it, because the list is what a fine-grained token can see rather than everything that exists, and GitHub refuses a taken name with a message that says so (P9-22). With the field empty, one button names a new repository for the Blueprint under the token’s own login, so no typing is needed at all. A repository with no commits — the one just created, or any other — previews against nothing and pushes a commit with no parent: GitHub answers its ref endpoints with `409 Git Repository is empty.` rather than 404, and the client treats that as "no branch", which to a preview is what it means (P9-23). Then **Preview the changes** builds a plan against that branch and shows it before anything can be pressed:

- every path that would be added, changed or removed — tagged `new`, `changed`, `removed` in the green, amber and red a diff reads by — with the ones edited on GitHub since the last push marked as such;
...
When a pushed target was packaged as a plugin (P9-27, P9-28, P9-36), the success panel shows the install commands with the repository just pushed to filled in — `/plugin marketplace add <owner>/<repo>` and `/plugin install <id>@<id>` for Claude Code, `codex plugin marketplace add <owner>/<repo>` for Codex, `copilot plugin marketplace add <owner>/<repo>` and `copilot plugin install <id>@<id>` for Copilot — since the dialog is the one place that knows the name. The README that goes up in that push names the repository too: the plan compiles with `repository: owner/name`, so its Installing section carries the real commands rather than the `<owner>/<repo>` placeholder an export to a ZIP has to leave (P9-33). The README is one of the compiler's own files, so it is rewritten on every push that changes it — unless the branch had a README before the first push ever ran, in which case it is listed under the files the app does not own and is written only when accepted.
