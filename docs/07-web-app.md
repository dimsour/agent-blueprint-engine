...
- **How to fix a finding** (P9-10): every finding — in the health bar, in the inspector, in an evaluation dimension, in the list blocking an export — carries an info control named `How to fix <code>`. Hovering or focusing it gives the code's one-line meaning; pressing it unfolds the remedy underneath and leaves it there. Both come from `DIAGNOSTIC_CODES` in core, so the guidance is written once per code and cannot differ between the four surfaces. A code the catalogue does not know renders no control rather than an empty panel.
