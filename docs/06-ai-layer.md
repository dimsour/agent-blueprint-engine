...
| `fixFinding`         | one `Diagnostic` from the validator or from a model, plus optional free text                                                                                      | the artifact in full **twice** — once as the selection, once beside the ask — its neighbours, the artifacts the finding names as `related`, the code's summary and remedy, the rule's invariant, the diagnostic's `data`, and the fields the code is about | `ChangeSet` with the `create` and `update` ops that clear that one finding, and a note saying whether the validator agrees it is cleared                                                         | schema; kinds narrowed per code; `fixabilityOf` refuses what no artifact edit can clear; the proposal is applied to a throwaway Blueprint and re-validated, and a finding still standing earns exactly one more attempt with the validator's own words |
