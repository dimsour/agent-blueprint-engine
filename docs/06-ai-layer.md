...
| `fixFinding`         | one `Diagnostic` from the validator or from a model                                                                                                               | the artifact the finding names, in full, plus its neighbours and the findings about it                  | `ChangeSet` with the `create` and `update` ops that clear that one finding                                                                                                                       | schema; the kinds the model may write are narrowed per code, and `fixabilityOf` refuses the codes no artifact edit can clear                                                         |
