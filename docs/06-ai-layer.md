...
| `addCapability`      | brief, optionally the selected agent                                                                                                                                                                                    | the whole Blueprint by name and one line each, the selected agent in full, existing findings                                                                                                                                                                                      | `ChangeSet` with `create` ops across every kind the capability needs, wired together and to what already exists — and **no** header op                                                                 | schema; ids deduped against the Blueprint; references resolved; checked against both validation passes with one repair round (P9-14); attaches to the selected agent only when the answer wrote no agent of its own                                                      |
...
| `fixFindings`        | one or more `Diagnostic`s from the validator or from a model, plus optional free text                                                                                                                                   | the artifact in full **twice** — once as the selection, once beside the ask — its neighbours, the artifacts the finding names as `related`, the code's summary and remedy, the rule's invariant, the diagnostic's `data`, and the fields the code is about                        | `ChangeSet` with the `create` and `update` ops that clear that one finding, each artifact once however many findings named it, and a note saying which of them the validator agrees are cleared        | schema; kinds narrowed per code; `fixabilityOf` refuses what no artifact edit can clear; the proposal is applied to a throwaway Blueprint and re-validated, and a finding still standing earns exactly one more attempt with the validator's own words                   |
| `fixBlueprint`       | nothing — everything the validator and the evaluation raise against the Blueprint as it stands (errors and warnings; suggestions stay with their rows), plus whatever the caller had, the twelve most severe in one ask | the Blueprint summary, every finding briefed as `fixFindings` briefs it, the artifacts the findings name as their files, and **how they are connected**: what each uses and is used by from the dependency graph, and for an orphan which agent holds which artifacts of its kind | `ChangeSet` with `create`, `update` **and `delete`** ops, a decision per finding — change, create, delete or keep, with the reason — as the first notes, and the validator's verdict on what it clears | schema; kinds narrowed per finding; a deletion is honoured only for an artifact nothing refers to, of an orphanable kind, and never an agent; an artifact returned half empty gets its fields back; the same throwaway-Blueprint check and single retry as `fixFindings` |
...
### What `fixFindings` is told, and why (P9-13, batched in P9-17)

A finding names a problem; clearing it needs the model to know what "fixed" means, where to
look, and what not to touch. All three were already written down in the product, and the
operation now sends every one of them:

| Sent                   | Where it comes from                                                                     | What it settles                                                                                                     |
| ---------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `summary`, `remedy`    | `DIAGNOSTIC_CODES` (core), `AI_DIAGNOSTIC_CODES` (ai)                                   | What the code means and how this kind of finding is fixed — the same words the user reads behind **How to fix**     |
| `invariant`            | `ValidationRule.description`, or `INVARIANT_BY_CODE` where one rule emits several codes | The exit condition. The finding is gone exactly when this is true again                                             |
| `evidence`             | the diagnostic's own `data`                                                             | The step, responsibility or reference the rule objected to — so a nine-step workflow does not have to be guessed at |
| `fields`               | `FIELDS_BY_CODE`                                                                        | Which fields to change, and by omission which to return untouched                                                   |
| `current`, `alsoNamed` | `renderEntity` over the `ref` and each `related`                                        | The artifact as its source file, repeated next to the ask rather than only thousands of tokens earlier              |
| `stillWrong`           | the validator, on a second attempt                                                      | What the first attempt failed to clear, and anything it broke                                                       |
| `instruction`          | the user, optionally                                                                    | What the catalogue could not know about this project                                                                |

The system prompt also carries one worked example — not per code, which would be a second
catalogue, but one, because it settles the two things every code shares and no schema states:
the answer is the whole artifact with one thing different, and everything the finding did not
mention comes back unchanged.

**A fix changes what the finding was about and nothing else** (P9-18). Told to add a `## Verification` section, a model will return the skill with the section added and `activation` emptied, `referenceIds` gone and `tags` dropped — it answers the question and deletes half the artifact doing it. The prompt says to copy every other field across, and a prompt is advice. The code holds it: when every finding about an artifact has a `fields` list in the core catalogue (`DiagnosticCode.fields`, one list read by the editor and the fix alike since P9-20), those fields are an allow-list and the proposal is merged onto the artifact as it stands, so everything outside the list comes back untouched — including what the model was never shown, since the AI schemas strip `metadata` and a workflow's node positions. Where a code names no fields (a contradiction, an orphan) there is no allow-list, and the weaker rule applies instead: a field that came back empty or missing is restored, a field that was filled in wins. Neither rule can invent content; both only give back what was already there, and both say in the notes what they kept.

A batch is not a loop over the single case, and the reason is not speed. Two findings so often name the same artifact — a skill with no `## Instructions` usually has no `## Verification` either — that fixing them one at a time means two edits of one file, the second computed from a Blueprint that does not yet have the first in it, so applying both loses one. One ask produces one version of that artifact with both addressed, which is also the only version a reviewer can sensibly read. The contract says so in those words, and `fixFinding` is the one-element case of `fixFindings`.

Then it checks. The proposal is applied to a throwaway Blueprint and `validateBlueprint` runs
again; if the finding still stands (or a new **error** appeared) the model gets one more
attempt with those exact words, and the better of the two answers is kept — better meaning it
clears the finding, or introduces less. The review says which happened, in words: _"Checked:
applying this clears BP-DESC-001"_, or that it does not. Codes the catalogue marks
`source: 'evaluation'` cannot be re-checked this way and say so rather than claiming success.
...
### What `fixBlueprint` decides, and what it is not allowed to (P9-40)

Reported from use: the assistant could clear one finding, or every finding of one dimension, and both are told to touch nothing else — right for a button beside a row, wrong for "what is wrong with this Blueprint, and what should I do about it?". The orphan Iron Law showed it. The per-finding fix attaches it to an agent and clears the warning; whether it _should_ be attached, duplicates a law the agent already has, or forbids something nothing in the Blueprint does, needs the whole design in view — and one honest answer is a deletion, which no per-finding fix proposes.

`fixBlueprint` is `fixFindings` with three things changed. Its input is not a list it was handed but everything both passes raise (`allFindings`), errors and warnings first and at most twelve per ask, the rest named for a second pass; suggestions stay with the row that raised them, because a whole-Blueprint pass over what is merely advisable is a rewrite. Its prompt asks for a **decision per finding** — `change`, `create`, `delete` or `keep`, with the reason — and those reasons are the first notes above the diff, so the reviewer reads _why_ before _what_. And it is given the evidence a decision needs: for every artifact a finding names, what it uses and what uses it from the dependency graph; for a law or a rule, what its own scope reaches, since that is what the compiler reads; and for an orphan, which agent holds which artifacts of its kind — the answer to "was it never wired, or is it not needed?" comes from that, not from the roster. The prompt says what a scope is for, tells the model to read what each law governs and who does that work, and names a blanket decision — the same action and reason on every finding — as no decision, because the first real run produced exactly that: twelve orphan laws, all set to apply to everything, one reason copied twelve times.

What it may not do is where the code, not the prompt, holds the line. A deletion is honoured only for an artifact that exists, of a kind `findOrphans` counts (`ORPHANABLE_KINDS`), that nothing refers to — the graph says, and a refusal names who still refers to it. An agent is never deleted here: a Blueprint with nobody in it is not a fix. An update to an artifact that is also being deleted is dropped in favour of the deletion, with a note. And an artifact the model returned to hold an orphan — an agent, usually — has no finding to say which of its fields were in play, so `restoringEmptied` applies the weaker half of the P9-18 rule to it: whatever came back empty or missing is restored from the original and noted. An artifact a finding _does_ name then goes through `draftsFrom` as in the single fix, so the catalogue's field list holds — a law asked about its `scope` does not come back with its severity raised, which the first run also produced. The rest is shared with `fixFindings` on purpose — the briefs, the artifact rendering, the throwaway-Blueprint check with one retry kept only when it is better.
