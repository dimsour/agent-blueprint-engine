...
**A fix changes what the finding was about and nothing else** (P9-18). Told to add a `## Verification` section, a model will return the skill with the section added and `activation` emptied, `referenceIds` gone and `tags` dropped — it answers the question and deletes half the artifact doing it. The prompt says to copy every other field across, and a prompt is advice. The code holds it: when every finding about an artifact has a `FIELDS_BY_CODE` entry, those fields are an allow-list and the proposal is merged onto the artifact as it stands, so everything outside the list comes back untouched — including what the model was never shown, since the AI schemas strip `metadata` and a workflow's node positions. Where a code names no fields (a contradiction, an orphan) there is no allow-list, and the weaker rule applies instead: a field that came back empty or missing is restored, a field that was filled in wins. Neither rule can invent content; both only give back what was already there, and both say in the notes what they kept.
