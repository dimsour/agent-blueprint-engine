 ...
 *
 * Reworded in the same task it was written, after the first real run: a model given twelve
 * orphan laws set every one of them to apply to everything, with one reason copied twelve
 * times. The prompt now says what a scope is for, tells it to read what each law governs, and
 * names a blanket decision as no decision. Still v1: the answer shape did not move and no
 * recording was pinned to the first wording.
 */
...
The typical case is an orphan: an artifact nothing uses. Ask why, one artifact at a time. Was
the agent that should hold it never wired to it? Then the fix is that agent, returned with the
artifact added to the right list — that is a change to the agent, not to the orphan. Does it
duplicate something the agent already has? Then delete it, and say which artifact already covers
it. Does nothing in this Blueprint do the thing it is about? Then delete it, and say so. Is it
plainly wanted and you cannot tell where it belongs? Then keep it and say what the author has to
decide. "How the artifacts are connected" is the evidence: read it before deciding.

An Iron Law or a rule is used by its own scope: the compiler puts it in the instructions of every
agent when the scope is "all", and of the named agents and workflows otherwise. So an orphan law
is one whose scope reaches nothing. The fix is to name the agents whose work it actually governs —
a law about EF Core transactions belongs to the agent that writes data access, not to a reviewer
of Blazor components — and to set it to all only when it is genuinely universal, such as never
faking verification or never exposing a secret. Read what each law is about and who does that
work; do not apply everything to everyone.

Each finding gets its own decision. The same action with the same reason on every finding is a
sign that nothing was decided; if twelve laws are orphans, some belong to one agent, some to
another, some to all, and some to nobody, and the answer says which.
