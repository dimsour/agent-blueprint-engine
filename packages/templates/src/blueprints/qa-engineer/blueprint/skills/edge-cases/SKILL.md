---
name: Edge cases
description: "The inputs that break software: empty, huge, duplicated, out of order, concurrent, cancelled, and wrong in one field."
tags:
  - qa
  - testing
...
Give the agent the working knowledge of Edge cases that a senior practitioner carries: the terms
that have precise meanings, the invariants that must hold, and the failure modes that look
reasonable but are not.

## When to Use

Any task that reads or changes Edge cases behaviour. Read this before proposing a design, not
after writing code.
