...
description: "Documenting an interface: parameters and their constraints, return values, errors, and the example that shows the common case."
tags:
  - api
  - documentation
...
Give the agent the working knowledge of API documentation that a senior practitioner carries: the terms
that have precise meanings, the invariants that must hold, and the failure modes that look
reasonable but are not.

## When to Use

Any task that reads or changes API documentation behaviour. Read this before proposing a design, not
after writing code.
