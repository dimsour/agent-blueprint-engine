...
tags:
  - security
whenToUse: At the start of an audit, and whenever a change adds an entry point or a trust boundary.
activation:
  intents:
    - security review
    - audit this
...
Give the agent the working knowledge of Threat modelling that a senior practitioner carries: the terms
that have precise meanings, the invariants that must hold, and the failure modes that look
reasonable but are not.

## When to Use

Any task that reads or changes Threat modelling behaviour. Read this before proposing a design, not
after writing code.
