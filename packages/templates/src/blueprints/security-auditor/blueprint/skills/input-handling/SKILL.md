---
name: Input handling
description: "Following untrusted input to the places it becomes dangerous: queries, commands, paths, templates and redirects."
tags:
  - security
...
Give the agent the working knowledge of Input handling that a senior practitioner carries: the terms
that have precise meanings, the invariants that must hold, and the failure modes that look
reasonable but are not.

## When to Use

Any task that reads or changes Input handling behaviour. Read this before proposing a design, not
after writing code.
