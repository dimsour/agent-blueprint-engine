---
name: Authorisation review
description: Checking who may call what, and whether the check is in a place that cannot be bypassed.
tags:
  - security
...
Give the agent the working knowledge of Authorisation review that a senior practitioner carries: the terms
that have precise meanings, the invariants that must hold, and the failure modes that look
reasonable but are not.

## When to Use

Any task that reads or changes Authorisation review behaviour. Read this before proposing a design, not
after writing code.
