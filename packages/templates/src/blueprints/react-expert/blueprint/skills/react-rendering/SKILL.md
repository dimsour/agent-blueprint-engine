...
description: "How React renders and re-renders: state, props, keys, memoisation, and the effects that should not exist."
tags:
  - frontend
  - react
whenToUse: When writing or changing a component, a hook, or anything that decides when a render happens.
activation:
  filePatterns:
    - "**/*.tsx"
    - "**/*.jsx"
...
Give the agent the working knowledge of React rendering that a senior practitioner carries: the terms
that have precise meanings, the invariants that must hold, and the failure modes that look
reasonable but are not.

## When to Use

Any task that reads or changes React rendering behaviour. Read this before proposing a design, not
after writing code.
