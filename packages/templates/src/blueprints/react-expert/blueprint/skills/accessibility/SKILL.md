---
name: Accessibility
description: "Semantics, keyboard operation and focus management: the parts of a component that decide whether it works for everyone."
tags:
  - accessibility
  - react
...
Give the agent the working knowledge of Accessibility that a senior practitioner carries: the terms
that have precise meanings, the invariants that must hold, and the failure modes that look
reasonable but are not.

## When to Use

Any task that reads or changes Accessibility behaviour. Read this before proposing a design, not
after writing code.
