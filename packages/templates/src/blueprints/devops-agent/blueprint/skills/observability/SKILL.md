---
name: Observability
description: "Making a change observable: what to log, what to measure, and the alert that would have caught this failure."
tags:
  - devops
  - operations
...
Give the agent the working knowledge of Observability that a senior practitioner carries: the terms
that have precise meanings, the invariants that must hold, and the failure modes that look
reasonable but are not.

## When to Use

Any task that reads or changes Observability behaviour. Read this before proposing a design, not
after writing code.
