...
description: "Declarative infrastructure: reading a plan before applying it, keeping state consistent, and making changes reversible."
tags:
  - devops
  - infrastructure
...
Give the agent the working knowledge of Infrastructure as code that a senior practitioner carries: the terms
that have precise meanings, the invariants that must hold, and the failure modes that look
reasonable but are not.

## When to Use

Any task that reads or changes Infrastructure as code behaviour. Read this before proposing a design, not
after writing code.
