---
name: Failure modes
description: "The ways code goes wrong in practice: boundaries, empty inputs, error paths, concurrency, and assumptions never checked."
tags:
  - correctness
  - review
...
Give the agent the working knowledge of Failure modes that a senior practitioner carries: the terms
that have precise meanings, the invariants that must hold, and the failure modes that look
reasonable but are not.

## When to Use

Any task that reads or changes Failure modes behaviour. Read this before proposing a design, not
after writing code.
